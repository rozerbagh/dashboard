            ...
            eightChar === false ? setError('Minimum Eight Character Required') :
                oneUC === false ? setError('At least one Capitalize character required') :
                    oneLC === false ? setError('At least one lowercase character required') :
                        oneSC === false ? setError('At least one special character required') : oneNum === false ? setError('At least one numeric value required') : setError(null)
...
const mapsDispatchToProps = dispatch => {
    return {
        onSettingPassword: (token, password, confirm_password) => dispatch(actions.set_password(token, password, confirm_password)),
