...
const headCells = [
    { id: 'lan_ip', numeric: false, disablePadding: true, label: 'Private IP' },
    { id: 'instance_name', numeric: false, disablePadding: true, label: 'Instances' },
];
...
function UserInstances(props) {
    ...
    const handleSelectAllClick = (event) => {
        if (event.target.checked) {
            const newSelecteds = props.instances.map((inst) => inst.lan_ip);
    ...
    useEffect(() => {
        sessionStorage.setItem('current_users_instances_ips', selected)
    }, [selected])

    useEffect(() => {
        let newSelected = [];
        props.instances.map((inst, index) => {
            if (inst.flag != 0) {
                newSelected.push(inst.lan_ip);
            }
        });
        setSelected(newSelected);
    }, [props.instances])
                        ...
                        <TableContainer>
                            <Table>
                                <EnhancedTableHead
                                    ...
                                    rowCount={props.instances.length}
                                />
                                <TableBody>
                                    {props.instances.map((inst, index) => {
                                        const isItemSelected = isSelected(inst.lan_ip);
                                        const labelId = `enhanced-table-checkbox-${index}`;
                                        return (
                                            <StyledTableRow
                                                hover
                                                onClick={(event) => handleClick(event, inst.lan_ip)}
                                                role="checkbox"
                                                aria-checked={isItemSelected}
                                                tabIndex={-1}
                                                key={`intance-of-${inst.lan_ip}-no-${index}`}
                    ...
                    <DialogActions>
                        <div className={classes.submitButton}>
                            <Button
                                ...
                                onClick={props.submitInstancesHandler}
