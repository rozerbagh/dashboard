...
import GeoTrafficTable from './GeoTrafficTable';
...
function GeoTraffic(props) {
    const classes = useStyles();

    const [ipCitiesList, setIpCitiesList] = useState([]);
    ...
    const fetchDataWithTiming = () => {
        ...
        if (fromTime > toTime) {
            ...
            setError(0);
            if (selectedValue == '') {
                // props.onFetchingGeoTrafficData(props.token, parseInt(fromTime / 1000), parseInt(toTime / 1000))
            } else {
                // props.onFetchingGeoTrafficDataWithIP(props.token, parseInt(fromTime / 1000), parseInt(toTime / 1000), ip)
            }
        }
    ...
    useEffect(() => {
        ...
        props.onFetchingGeoTrafficData(props.token, from / 1000, to / 1000);
        setFirstRun(true)
    }, []);
    ...
    useEffect(() => {
        const arr = [];
        let wanips = [];
        props.ipCitiesList.map((listItem, index) => {
            ...
            <div className={classes.marginTop10}>
                ...
                {props.loader ?
                    <div className={classes.loaderContainer}>
                        ...
                        {ipCitiesList.map((list, i) => {
                            let ipTrackArr = [];
                            let total = 0;
                            let total_recived = 0;
                            let total_transmit = 0;
                            return <Card key={`${list.wan_ip}-ip_city-table-${i}`}>
                                ...
                                <GeoTrafficTable wan_ip={list.wan_ip} rows={ipTrackArr}
                                    csvNo={i}
...
const mapStateToProps = (state) => {
    return {
        ...
        ipCitiesList: state.customerReports.ip_cities_list,
        loader: state.customerReports.loading,
        error: state.customerReports.ip_cities_error,
    }
}

const mapDispatchToProps = (dispatch) => {
    return {
        onFetchingGeoTrafficData: (token, from, to) => dispatch(action.fetchGeoTrafficData(token, from, to)),
        onFetchingGeoTrafficDataWithIP: (token, from, to, ip) => dispatch(action.fetchGeoTrafficDataWithIP(token, from, to, ip)),
    }
}

export default connect(mapStateToProps, mapDispatchToProps)(GeoTraffic);