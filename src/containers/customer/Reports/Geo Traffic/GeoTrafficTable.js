    ...
    return (<>
        <TableContainer>
            <div className={classes.tableTitleContainer}>
                <span style={{ fontWeight: 500, fontSize: "0.8rem" }}>City wise Traffic
                    | {props.wan_ip}</span>
                <div>
                    ...
                    {props.rows.map((row) => (
                        <StyledTableRow key={`${props.wan_ip}-${row.id}`}>
                            <StyledTableCell align="left">
                                <span style={{ paddingLeft: "20px" }}></span>{row.label}
                            </StyledTableCell>
                            <StyledTableCell align="left">{row.public_ip}</StyledTableCell>
