...
export const postDailyReports = (token, cxo_report, cxo_report_email, tech_report, tech_report_email) => {
    return dispatch => {
        ...
        dispatch(post_daily_report_success('Daily Reports has been sheduled'));
        setTimeout(() => dispatch(nullifyingDRSucMsg()), 5000)
        // dispatch(post_daily_report_fail(error.response.data.message));
    }
}

export const post_notify_report_start = () => {
    return {
        type: actionType.POSTING_NOTIFICATIONS_START
    }
}
export const post_notify_report_success = (success_message) => {
    return {
        type: actionType.POSTING_NOTIFICATIONS_SUCCESS,
        successMsg: success_message
    }
}

export const post_notify_report_fail = (error) => {
    return {
        type: actionType.POSTING_NOTIFICATIONS_FAIL,
        errorMsg: error,
    }
}

export const nullifyingNotiSucMsg = () => {
    return {
        type: actionType.POSTING_NOTIFICATIONS_SUCCESS,
        successMsg: null
    }
...
export const nullifyingNotiErrMsg = () => {
    return {
        type: actionType.POSTING_NOTIFICATIONS_FAIL,
        errorMsg: null,
    }
...
export const postNotifyReports = (token, notify_report_email) => {
    return dispatch => {
        dispatch(post_notify_report_start());
        dispatch(post_notify_report_success('Notifications has been sent'));
        setTimeout(() => dispatch(nullifyingNotiSucMsg()), 5000)
        // dispatch(post_notify_report_fail(error.response.data.message));
    }
}