...
export const fetchInstanceVmInfo = (token, ip) => {
    return dispatch => {
        ...
        vm_info.message.map(ele => {
            ele.lan_ip = '88.888.8.888';
            ele.vm_hostname = 'Demo - Instances';
            ele.vm_server_hostname = 'Demo - Instances';
            ele.wan_ip = '77.777.7.77';
        })
        ...
        services.message.map(ele => {
            ele.srvices.map((e, i) => {
                if (i === 0 || i === 1) {
                    ...
                    e.title = 'demo components';
                    e.subtitle = ''
                    e.details = 'Demo Services details';
                }
            })
...
export const fetchISPContDetails = (token, ip) => {
    return dispatch => {
        ...
        isp_contributor.message.map(ele => {
            ele.label = '222.222.222.222\\Demo-IN';
            ele.description = '\\City Company Pvt Ltd, Country Code\\';
        })
...
export const fetchZoomViewTask = (token, ip) => {
    return dispatch => {
        ...
        tasks.message.map(ele => {
            ele.host_name = 'Demo - Instances';
            ele.created_by = 'Demo Person'
        })
...
export const fetchZoomViewInstances = (token, from, to, lip, wip) => {
    ...
    return dispatch => {
        ...
        instnaces.message.map(inst => {
            inst.lan_ip = '88.888.8.88';
            inst.wan_ip = '77.777.7.77';
